// Sprint 10 — /account/schedules: beheer audit-reminders.
//
// Server Component: laadt reminders, render SchedulesList (client) voor
// toggle/delete-acties.

import Link from 'next/link';
import { redirect } from 'next/navigation';
import { ShoppingCart, ArrowLeft, Bell } from 'lucide-react';
import { createClient } from '@/lib/supabase/server';
import { listMyReminders } from '@/lib/audit-reminders';
import SchedulesList from './SchedulesList';

export const metadata = {
  title: 'Audit-reminders — Conversielek',
};
...
export default async function SchedulesPage() {
  ...
  if (!user) redirect('/login?error=' + encodeURIComponent('Log in om reminders te beheren.'));
          ...
          <div className="flex items-start gap-3 mb-8">
            ...
            <div>
              ...
              <p className="text-slate-600 dark:text-slate-400 mt-1 text-sm leading-relaxed">
                Krijg een email-herinnering om periodiek een nieuwe audit te doen, plus
                automatische waarschuwingen wanneer je score zakt.
              </p>
            </div>
          </div>

          <SchedulesList initialReminders={reminders} />
        </div>
      </div>
    </div>
  );
